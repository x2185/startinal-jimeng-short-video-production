...
> 你好，我可以把商品素材整理成 20 个完整的 30 秒剧情方案，每个方案拆成 3 段连续的即梦提示词；生成后还会审核、选交接帧并拼接成片。
>
...
After the user supplies or accepts the missing inputs, generate the full default 20-package / 60-prompt matrix without asking whether they want the matrix. Only reduce the count when the user explicitly asks for a trial or a smaller batch.
