...
When the user specifically needs a fast, causally connected story rather than a showcase sequence, treat **25 seconds as the default finished-video target for this high-density route**, not as a fixed single-clip duration. Standard variant packages retain their separately stated 30-second default unless the user specifies another target. Read the live Dreamina CLI help and choose the requested model first. Use one continuous 25-second validation clip only when that exact model supports it and the product/action type is stable enough; otherwise split the same 25-second beat map into model-supported, causally linked source clips and assemble them. Never force either a long take or five-second clips merely because a different model supports them.

Keep the story continuous at the macro level, but organise it into 1–3-second microbeats. Each microbeat must have one actor intention, one visible action or reaction, one camera instruction, and a clear reason it leads into the next beat. Use motivated transitions (match action, foreground occlusion, a door/counter pass, camera follow, or an ordinary cut). Do not promise frame-perfect timing: the model may vary timing, so reserve exact one- or two-second cuts, text, captions, sound effects, and beat accents for deterministic finishing.

Build a role-labelled, same-SKU reference pack up to the selected model's live input limit; do not upload historical generated output as new product evidence. Use as many distinct, useful originals as needed to cover identity (full front, back, side, material/detail), action (start, contact/operation, end), and linked-clip continuity (approved outgoing handoff frame). Do not waste input capacity on near-duplicates, other colourways, or images that reveal sensitive card/account data. For every paid continuous-story test, prepare a time-coded director prompt, validate the returned clip for causal clarity and identity stability, and only then scale the route.

Read `references/high-density-continuous-story.md` before planning or submitting this route. Read the live `dreamina multimodal2video -h` before every CLI submission; the live help determines model, input, duration, resolution, and ratio limits. Immediately before upload, state the selected references, model, duration, resolution, ratio, output count, and known or unknown credit impact, then obtain explicit approval.

For a quick UGC story whose tension is whether a novelty cardholder carrying the user's own tap-enabled card will work at checkout, read `references/fast-ugc-payment-proof-structure.md`. Treat past successful videos only as rhythm learning: do not reuse their footage, actors, brands, specific settings, dialogue, or claimed transaction result.

Do not make any historical product, setting, relationship, action, or story engine the default for later jobs. Start every new product from its current identity passport, verified function, available action evidence, buyer moment, and restrictions. A case-specific reference is an optional pattern only when its product facts and action class match the current brief; otherwise use its abstract pacing principles and create a new story premise.

## Retained legacy API route

Keep the existing API-first production route available for users who explicitly select it or whose task is better suited to its supported single-reference, independently generated scene blocks. Read `references/api-first-batch-production.md` before using that route; preserve its own model limits, approval gates, and production rules. The Dreamina CLI high-density route adds a separate multi-reference option—it does not delete, replace, or silently reroute an explicitly requested legacy API workflow.

When the user names a route, model, duration, or source-clip structure, follow it unless it is genuinely unsupported; state the precise limitation and offer the smallest compatible alternative. When no route is specified, proactively ask one compact routing question before drafting a production package: whether to use Dreamina CLI for multi-reference/high-density continuity or the retained legacy API for short independent scene blocks. State the current brief's likely fit and the practical tradeoff, but do not silently select a route from a historical default. Skip this question only when the current conversation has already locked a route.

When the user requests creative, high-density variants, vary the causal premise—not just styling—across opening event, character goal, relationship, pressure, setting, product-proof moment, turn, outcome, camera logic, and ending. Each variant must retain a readable cause-and-effect chain and product evidence while advancing through meaningful 1–3-second beats; do not pad a fast brief with repeated product poses, generic b-roll, or adjective-only rewrites.

When the user supplies a prompt, script, shot list, dialogue, visual reference, or successful prior direction, treat it as the highest-weight creative source. Preserve its stated premise, story order, character relationships, pacing, tonal intent, must-show beats, and requested ending unless the user asks to replace them. Treat its **instruction granularity** as a quality reference too: if it specifies 0.5–2-second actions, prop placement, micro-reactions, shot order, dialogue beats, and transition actions, match that operational detail in the new package. Audit only against the current product evidence, duration, safety restrictions, and model feasibility; make the smallest needed correction and label it. Derive variants from the user's core dramatic engine before consulting optional Skill libraries. Do not flatten a supplied script into a generic product showcase or silently replace its creative decisions with a default template.
...
5. Choose the shot plan.
   - For a **25-second high-density finished video**, first select the model and inspect its live limits. Use one 25-second continuous story only when that exact model supports 25 seconds and its test risk is acceptable. Otherwise create linked source clips whose durations add to 25 seconds (for example `5 × 5s` or `10s + 10s + 5s`), with an explicit handoff at every join. Do not convert a 25-second brief into unrelated clips or a 30-second package merely to fit an older default.
   ...
   - Do not choose a duration merely to fill an old 30-second template. Choose model-supported source durations that preserve the requested finished duration, put one clearly verifiable action in each source clip, and leave a stable handoff/end state.
...
7. When the user asks for variants without supplying a selected script, derive the default matrix of 20 complete 30-second creative packages from the current product brief and supplied assets. Mix suitable modes—story, gameplay/function proof, visual product showcase, interaction/reaction, and hook/reveal—instead of forcing every package into a plot. When the user supplies a selected script, preserve it as the base package and derive only the requested variants. Do not reuse stories, characters, settings, or dialogue from a prior product unless the user asks. For the explicitly requested high-density route, prepare one 25-second finished-video validation package, then choose its single-clip or linked-clip source route from the selected model's live limits and expand only after it passes review. **A requested batch is not permission to lower the package grade:** label a short premise/beat matrix `creative-outline`; label each variant `director-grade` only when it independently contains the complete continuity bible, reference-role plan, time-coded beats, source-route/tail-frame plan, full submission prompt, targeted negatives, acceptance gate, and a separately completed all-pass record from `references/director-grade-benchmark-checklist.md`.
...
8a. When generated story scenes need a real product to remain visually exact, build a **product reference pack** from the best verified identity frames before prompt drafting. Assign every item a role—identity, reverse/geometry, material/detail, action start, action contact, action end, or approved handoff—and include the role in the submission prompt. Preserve original source frames, generate transparent cutouts only when the local background-removal dependency is available and the result passes edge review, and label every cutout as an identity asset—not proof of an unseen action. Use the pack for deterministic compositing or as a staged first-frame reference. Do not ask image-to-video to recreate fine product geometry, conceal a failed mechanical action, or simulate hand occlusion it cannot preserve.
...
14. For every linked source clip, write a **tail-frame contract before submission**: ending composition, actor pose/eyeline, product location/orientation, held props, setting, lighting, camera angle, and the next clip's opening action. After generation and before its continuation, run the reference-frame gate: extract multiple candidates from the final stable portion, inspect them, and select a product-visible handoff frame that meets that contract. Never automatically use the literal last frame. A candidate that changes the product identity passport is ineligible. Attach the approved extracted tail frame plus the fixed product-reference pack to the next submission whenever the selected model supports it.
15. Keep each clip independently generatable. For a stable product display, reuse the fixed product-identity reference throughout whenever the API supports multiple references; otherwise, carry the approved handoff frame forward and repeat the complete identity passport in the prompt. For a rotation, opening, card insertion, assembly, insertion, or other state-changing action, use JiMeng's **All-reference** submission mode and attach a clip-specific evidence set: completed product identity; exact start state; one or more close views of the mechanism/detail; and exact end state. Name each uploaded item in the prompt (`@image-1`, `@image-2`, and so on) with its permitted role. Do not send such a clip through the legacy `jimeng_i2v_first_v30` runner, which accepts only one first-frame image. Never allow a continuation to substitute a different product, colorway, packaging, logo, material, or included part.
...
17. For an approved **single-reference, low-risk** API run, use `scripts/run_jimeng_a1_package.py` with its default `--download-route auto`. It retries the normal route and a proxy-bypassing direct route, refreshes an expired signed CDN URL once, and resumes completed local clips without resubmitting or spending again. For an action that needs several state references, prepare the All-reference upload plan in `references/multi-reference-action-submission.md` and submit only through the compatible JiMeng UI; this runner intentionally rejects a multi-image package. For a named, explicitly approved test-only package, use `--max-clips 1` to validate the highest-risk first action before paying for the remaining linked clips; the runner must skip final assembly for a partial test and the package must guard against accidental full submission. Do not ask the operator to manually download a clip before those recovery steps have failed.
