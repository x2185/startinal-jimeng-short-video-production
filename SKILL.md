...
6. Build the submission package: identity passport, reference roles, visible detail proof, continuity/tail-frame plan, prompt, targeted negatives, acceptance checks, and a concise change log. Run `references/pre-submission-prompt-optimization.md` and `references/prompt-quality-gate.md` before presenting it. Label any incomplete concept `creative-outline`; it is planning only and cannot be submitted.
7. Use two unambiguous approval stages. For a `creative-outline`, state that it is not submission-ready, consumes no credits, and that confirmation authorizes only expansion into a director-grade package. Only a displayed `director-grade / submission-ready` package may request paid approval; state the exact model, references, duration, resolution, ratio, output count, known credit impact, and that the named confirmation will submit it now. Never submit drafts, changed scope, or silence.
