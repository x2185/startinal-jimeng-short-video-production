...
15. Keep batch clips independently generatable by default: reuse the fixed product-identity reference for each low-risk cut so a malformed previous frame cannot contaminate later clips. Use an approved handoff only when the continuity benefit is necessary and Codex's visual review has passed the source clip. For a rotation, opening, card insertion, assembly, insertion, or other state-changing action, use a compatible multi-reference API only when its input contract has been verified for that job; otherwise use verified real footage. Use JiMeng's **All-reference UI** only when the user explicitly chooses the UI fallback. Attach only same-SKU evidence: completed product identity; exact start state; one or more close views of the mechanism/detail; and exact end state. Name each uploaded item in the prompt (`@image-1`, `@image-2`, and so on) with its permitted role. Do not send such a clip through the legacy `jimeng_i2v_first_v30` runner, which accepts only one first-frame image. Never allow a continuation to substitute a different product, colorway, packaging, logo, material, or included part.
