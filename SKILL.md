...
For the legacy 720P single-first-frame API, use a **scene-block production model**: batch-generate varied 5-second low-risk shots, then assemble the selected blocks into a 30-second final. Do not request a 30-second continuous performance from this API. Treat actual payment contact, card loading, hidden-slot operation, twisting, assembly, and similarly exact state-changing demonstrations as real-footage inserts unless a compatible multi-reference API has been verified for the job.
...
Greet the user briefly and explain that the skill first locks product evidence and validates one representative high-risk or product-critical clip, then expands accepted structures into a batch. Inspect existing conversation/files first, then collect only missing product identity, play/use, verified features, buyer/market, restrictions, and extra creative requirements. When the user supplies a script, first run the script-assist pass in `references/script-assist-and-variants.md`: preserve its approved intent and structure, but verify every claim, action, and visual against supplied evidence before optimizing it or deriving variants. Unless already explicitly supplied, always ask a separate question for: verified selling points and the core functions/features that must be shown and preserved. Do not infer these from video alone. Produce 20 variants only when the user asks for creative/batch expansion; never make a large prompt matrix substitute for a successful visual test.
...
5. Choose the shot plan and execution channel.
   - For API-first work, make each batch independently resumable. Begin with one named high-risk test clip, automatically extract its review frames, and have Codex perform the visual acceptance gate. Ask the user only when the result is failed/ambiguous, new source material is needed, or a new paid scope needs approval. Then submit accepted low-risk/stable clips in a controlled batch after the user approves their IDs.
...
6. Run the action-and-state reference gate before drafting prompts. For every must-show interaction, create the action map and prompt-asset mapping required by `references/action-and-prompt-mapping.md`: product identity, starting state, **one** permitted action, ending state, source assets, and acceptance criteria. Prefer original action footage or action-specific stills over a packshot. A packshot establishes appearance only; it cannot prove an unseen mechanism. Redact or avoid source material with readable personal data, card data, brands, prices, or prohibited content. If the action has no evidence, ask for it or omit the action—never have the model invent a hidden mechanism.
7. Create one production-ready validation package from the current product brief and supplied assets. When the user asks for creative/batch expansion after an accepted structure is available, derive 20 complete 30-second packages from that structure. When an optimized user script is available, include it as one base route and derive the remaining routes from its validated insight, not by making superficial wording swaps. Mix suitable modes—story, gameplay/function proof, visual product showcase, interaction/reaction, and hook/reveal—instead of forcing every package into a plot. Do not reuse stories, characters, settings, or dialogue from a prior product unless the user asks.
...
8a. When generated story scenes need a real product to remain visually exact, build a **product reference pack** from the best verified identity frames before prompt drafting. Preserve original source frames, generate transparent cutouts only when the local background-removal dependency is available and the result passes edge review, and label every cutout as an identity asset—not proof of an unseen action. Use the pack for deterministic compositing or as a staged first-frame reference. Do not ask image-to-video to recreate fine product geometry, conceal a failed mechanical action, or simulate hand occlusion it cannot preserve. When the operator wants creative scene variation, apply `references/creative-first-frame-workflow.md`: product material locks identity and real actions; new AI-created or licensed backgrounds supply scene, mood, and camera intent. Do not reduce the output to repeated copies of a raw product photo merely to keep it safe.
...
14. Run the reference-frame gate automatically after each generated clip. The runner must extract start, middle, action, and ending review frames, then Codex must inspect them against the product-proof acceptance gate. Never automatically use the literal last frame. A candidate that changes the product identity passport is ineligible. Escalate to the user only when no candidate passes or the judgement is genuinely ambiguous.
15. Keep legacy 720P single-first-frame batch clips independently generatable: upload exactly one fixed target-SKU product-identity image for each low-risk cut. Its prior clip's tail frame is a review/edit reference only and must not be uploaded alongside the identity image, because this API cannot accept both. Do not automatically replace the fixed identity image with a generated tail frame. Use an approved handoff as an additional input only in a compatible multi-reference API. For a rotation, opening, card insertion, assembly, insertion, or other state-changing action, use a compatible multi-reference API only when its input contract has been verified for that job; otherwise use verified real footage. Use JiMeng's **All-reference UI** only when the user explicitly chooses the UI fallback. Attach only same-SKU evidence: completed product identity; exact start state; one or more close views of the mechanism/detail; and exact end state. Name each uploaded item in the prompt (`@image-1`, `@image-2`, and so on) with its permitted role. Do not send such a clip through the legacy `jimeng_i2v_first_v30` runner, which accepts only one first-frame image. Never allow a continuation to substitute a different product, colorway, packaging, logo, material, or included part.
...
17. For an approved **single-reference, low-risk** API run, use `scripts/run_jimeng_a1_package.py` with its default `--download-route auto` and `continuity_mode: independent`. It reads the approved local reference image directly and sends Base64 to the API; it needs no JiMeng web login. It retries the normal route and a proxy-bypassing direct route, refreshes an expired signed CDN URL once, creates six inspection frames per clip, and resumes completed local clips without resubmitting or spending again. Its assembled file is a preview pending Codex visual QA, not automatic final acceptance. For an action that needs several same-SKU state references, use a verified compatible API route; if none is configured, use real footage or offer the UI fallback only when the user asks for it. For a named, explicitly approved test-only package, use `--max-clips 1` to validate the highest-risk first action before paying for the remaining linked clips; the runner must skip final assembly for a partial test and the package must guard against accidental full submission. Do not ask the operator to manually download a clip before those recovery steps have failed.
...
19. Deliver a finished 30-second vertical video by default, not a folder of generated fragments. Treat the low-risk generated blocks and any approved real-action insert as source material only. Automatically select accepted sources, arrange them into the approved hook → context/proof → real-action insert when required → hero ending sequence, and render the final MP4. Return source clips separately only when the user explicitly requests raw material or the final is blocked by a missing required real-action asset.
...
23. Before a new API batch, inspect the approved generated-media library only to learn tested prompt structure: source path, SKU, beat, observed camera/action/ending, prompt provenance (`recorded` or `inferred`), and known failure exclusions. Every deliverable clip must be newly generated for the current scene and brief; never directly reuse an earlier generated clip in a new final video unless the user explicitly asks to reuse that exact clip. An inferred record is a reusable template, never a claim that the exact historical prompt has been recovered.
...
Read `references/action-and-prompt-mapping.md` before drafting prompts for a product action or choosing a substitute beat.
...
- For a legacy 720P single-first-frame batch, treat the 30-second story as an edit plan, not a continuous generated take. Generate separate 5-second blocks with different supported settings (for example carry, tabletop reveal, material macro, social setting, hero display), one low-risk movement per block, then insert any approved real action footage at its planned beat.
- For a verified interaction, describe the exact evidence asset, starting state, one primary visible action, and required ending state. Do not compress multiple fine hand actions into a single generated clip. A later generated continuation is allowed only in a verified multi-reference route; in the legacy 720P route, insert real footage for the interaction or make an intentional cut to an independent scene block.
...
4. Use explicit visual handoffs only for a verified multi-reference route. For legacy 720P single-first-frame work, design intentional hard cuts between independent scene blocks: each block repeats the fixed product identity but must not claim the same actor pose, camera position, or moment continues from the previous block.
...
4. One validation package by default. When the user requests expansion, a mode-balanced matrix of **20 complete 30-second creative packages**, with mode, route, full arc or proof progression, visible proof, difference, asset requirement, and its linked prompts.
5. One complete JiMeng prompt per generated clip. Include a prior-clip handoff only when the selected API accepts that additional reference; otherwise label the edit as an intentional scene cut.
