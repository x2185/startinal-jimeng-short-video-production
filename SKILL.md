...
When the user names a route, model, duration, or source-clip structure, follow it unless it is genuinely unsupported; state the precise limitation and offer the smallest compatible alternative. When no route is specified, select between API and CLI from the current brief, available references, model support, and needed continuity—not from a historical default. Use CLI when multi-reference control or a high-density linked story materially helps; retain API for its existing short, independent scene-block workflow.
...
- Put only one high-risk physical interaction in a microbeat. Rewrite unsupported fine operations as a safe movement, reaction, product reveal, or named real-footage insert. Do not ask the generative model to render exact on-screen text, captions, or frame-perfect edit timing.
